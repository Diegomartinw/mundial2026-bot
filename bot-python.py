...
FIFA_LEAGUE      = 1    # Mundial 2026
FRIENDLY_LEAGUE  = 9    # Amistosos internacionales
SEASON           = 2026
...
def get_fixtures(date_str):
    try:
        all_fixtures = []
        for league in [FIFA_LEAGUE, FRIENDLY_LEAGUE]:
            url = f"https://v3.football.api-sports.io/fixtures?league={league}&season={SEASON}&date={date_str}"
...
# ── NIGHTLY SUMMARY ───────────────────────────────────────────────────────────
def send_nightly_summary(fixtures):
    played = [f for f in fixtures if f["fixture"]["status"]["short"] in {"FT","AET","PEN"}]
    if not played:
        send_wa("🌙 *Resumen del día:* No hubo partidos del Mundial hoy.")
        return
    msg = "🌙 *Resultados del día — Mundial 2026:*\n\n"
    for f in played:
        home   = f["teams"]["home"]["name"]
        away   = f["teams"]["away"]["name"]
        ...
        extra  = " *(penales)*" if status == "PEN" else " *(prórroga)*" if status == "AET" else ""
        msg += f"{flag(home)} {home} *{g_home} - {g_away}* {away} {flag(away)}{extra}\n"
    msg += "\n¡Hasta mañana! 🏆"
    send_wa(msg)
...
def main():
    ...
    while True:
        ...
        # Resultados de ayer a las 7 AM
        if now.hour == 7 and now.minute == 0 and yesterday_sent != date:
            yesterday_sent = date
            fixtures_ayer = get_fixtures(yesterday_str())
            send_yesterday_summary(fixtures_ayer)
        ...
        # Resumen 23:00
        if now.hour == 23 and now.minute == 0 and nightly_sent != date:
            nightly_sent = date
            fixtures = get_fixtures(date)
            send_nightly_summary(fixtures)
